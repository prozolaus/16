...
#define ARRAY_SIZE(a) (sizeof(a) / sizeof((a)[0]))
    ...
    pair<double, double> line_intersect(Point p1, Point p2, Point p3, Point p4)
    {
        double x1 = p1.x;
        double x2 = p2.x;
        double x3 = p3.x;
    ...
    void Striped_closed_polyline::draw_borders() const
    {
        if (1 < number_of_points()) // draw sole pixel?
    ...
    pair<int, int> Striped_closed_polyline::get_min_y() const
    {
        int min_y = point(0).y, min_y_p = 0;
        ...
        return pair<int, int>(min_y, min_y_p);
    }

    //-----------------------------------------------------------------------------------

    pair<int, int> Striped_closed_polyline::get_max_y() const
    {
        int max_y = point(0).y, max_y_p = 0;
        for (int i = 1; i < number_of_points(); i++)
            if (max_y < point(i).y)
            {
                max_y = point(i).y;
        ...
        return pair<int, int>(max_y, max_y_p);
    }

    //-----------------------------------------------------------------------------------

    void Striped_closed_polyline::fill_vpl_vpr(vector<Point> &vpl, vector<Point> &vpr, int min_y_p, int max_y_p) const
    {
        for (int i = 0; i < number_of_points(); i++)
        {
            if (i != min_y_p && i != max_y_p)
            {
                if (point(i).x < point(min_y_p).x || point(i).x < point(max_y_p).x)
                    vpl.push_back(point(i));
                else
                    vpr.push_back(point(i));
            }
        }

        vpl.push_back(point(max_y_p));
        vpr.push_back(point(max_y_p));

        //sort points along the y-axis
        stable_sort(vpl.begin(), vpl.end(), [](const Point &a, const Point &b)
                    { return a.y < b.y; });
        stable_sort(vpr.begin(), vpr.end(), [](const Point &a, const Point &b)
                    { return a.y < b.y; });
    }

    //-----------------------------------------------------------------------------------

    void Striped_closed_polyline::fill() const
    {
        fl_color(fill_color().as_int());

        //define the highest point (with minimal y) and the lowest one (with maximal y)
        pair<int, int> pair_min{get_min_y()};
        pair<int, int> pair_max{get_max_y()};
        int min_y{pair_min.first},
            min_y_p{pair_min.second},
            max_y{pair_max.first},
            max_y_p{pair_max.second},
            min_x = 0,
            max_x = 1920;

        vector<Point> vpl{point(min_y_p)}, vpr{point(min_y_p)}, vplf, vprf;

        fill_vpl_vpr(vpl, vpr, min_y_p, max_y_p);

        const int step = 10; //shading step in pixels

        for (int i = step, j = 0, k = 0, count = 0; i < max_y - min_y; i += step)
        {
            Point p1{min_x, min_y + i}, p2{max_x, min_y + i};

            if (vpl[j].y == vpl[j + 1].y)
                j++;

            pair<double, double> u = line_intersect(p1, p2, vpl[j], vpl[j + 1]);

            //if there was no intersection
            if (u.first < 0 || u.first > 1 || u.second < 0 || u.second > 1)
            {
                j++; //next segment left of center
                i -= step;
                continue;
            }
            //if the intersection was
            else
                vplf.push_back(Point{int(p1.x + u.first * (p2.x - p1.x) + 1), int(p1.y + u.first * (p2.y - p1.y))});

            Point pis1{vplf[count].x, vplf[count].y}; //start fill point

            if (vpr[k].y == vpr[k + 1].y)
                k++;

            pair<double, double> u2 = line_intersect(pis1, p2, vpr[k], vpr[k + 1]);
            if (u2.first < 0 || u2.first > 1 || u2.second < 0 || u2.second > 1)
            {
                k++; //next segment right of center
                i -= step;
                vplf.pop_back();
                continue;
            }
    ...
    void Striped_closed_polyline::draw_lines() const
    {
        if (number_of_points() < 3)
            error("Added less than three points!");
